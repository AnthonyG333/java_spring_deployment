	...
	<div class="col p-0" id="column2">
		...
		<!-- ALL PUBLIC MESSAGES -->
			...
			<c:choose>
				...
				<c:when test="${currUser.replies.size() == 0}">
					<c:choose>
						<c:when test="${currUser.id == user_id}">
							<p class="text-danger font-weight-bold m-2"> You have not posted any replies. </p>
						</c:when>
						<c:otherwise>
							<p class="text-danger font-weight-bold m-2"> <c:out value="${currUser.userName}"/> has not posted any replies. </p>
						</c:otherwise>
					</c:choose>
				</c:when>
			
				<c:otherwise>	
					<c:forEach var="message" items="${currUser.replies}" >
					...
					<div id="buttons">
						<a href="/liaison/reply/get/${message.publicMessage.id}" class="btn-sm btn-success rounded">️  Parent Message </a>
				 	</div>
